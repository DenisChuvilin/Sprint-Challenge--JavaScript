...
const unisWithUni = [];
for (i = 0; i < graduates.length; i++) {
  if (graduates[i].university.includes('Uni')) unisWithUni.push(graduates[i].university);
}
...
zooAnimals.filter(animal => {
  if (animal.population < 5) lowPopulationAnimals.push(animal);
});
...
zooAnimals.forEach(object => newarray.push(object.population));

const populationTotal = newarray.reduce((accumulator, currentValue) => {
